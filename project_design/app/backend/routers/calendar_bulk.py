...
class RepairScheduleRequest(BaseModel):
    project_id: Optional[int] = None  # None이면 전체 프로젝트 대상


class RepairScheduleResponse(BaseModel):
    checked_staffings: int
    missing_mapped: int   # entries는 있는데 staffing_id가 없거나 매핑 끊긴 케이스
    orphan_entries: int   # staffing이 삭제됐는데 entries 남아있는 경우
    orphan_deleted: int   # 고아 entries 실제 삭제 수
    detail: List[str]


@router.post("/repair_schedule", response_model=RepairScheduleResponse)
async def repair_schedule_mapping(
    request: RepairScheduleRequest,
    db: AsyncSession = Depends(get_db),
):
    """
    DB에 저장된 calendar_entries 기준으로 인력별 일정 표시가 안 되는 원인을 진단하고
    고아(orphan) entries를 정리합니다.

    원인 ①: staffing.deleted_at이 설정됐는데 calendar_entries가 남아 있음
             → orphan entries 삭제
    원인 ②: calendar_entries.staffing_id가 실제 staffing 레코드에 없음
             → orphan entries 삭제
    원인 ③: phase가 현재 월에 겹치지 않아 visiblePhases에서 제외됨
             → 진단 정보만 반환 (프론트 로직 문제)
    """
    detail: List[str] = []

    # 유효한 staffing_id 목록 (deleted_at is None)
    if request.project_id:
        ...
        staffing_stmt = select(Staffing).where(Staffing.deleted_at.is_(None))

    staffing_result = await db.execute(staffing_stmt)
    valid_staffings = staffing_result.scalars().all()
    valid_ids = {s.id for s in valid_staffings}
    detail.append(f"유효한 staffing 레코드: {len(valid_ids)}개")

    # 전체 calendar_entries 중 staffing_id가 valid_ids에 없는 것 (고아)
    if request.project_id:
        # 해당 프로젝트의 staffing ids (삭제 포함)
        all_proj_staffing = await db.execute(
            select(Staffing.id).where(Staffing.project_id == request.project_id)
        )
        all_proj_ids = {r[0] for r in all_proj_staffing.fetchall()}
        orphan_stmt = select(Calendar_entries).where(
            Calendar_entries.staffing_id.in_(all_proj_ids),
            Calendar_entries.staffing_id.not_in(valid_ids),
        )
    else:
        # 전체 staffing ids (삭제 포함)
        all_staffing_res = await db.execute(select(Staffing.id))
        all_staffing_ids_set = {r[0] for r in all_staffing_res.fetchall()}
        orphan_stmt = select(Calendar_entries).where(
            Calendar_entries.staffing_id.not_in(all_staffing_ids_set),
        )

    orphan_result = await db.execute(orphan_stmt)
    orphan_entries = orphan_result.scalars().all()
    orphan_count = len(orphan_entries)
    detail.append(f"고아 calendar_entries (staffing 삭제됨): {orphan_count}개")
    ...
    # soft-deleted staffing에 연결된 entries도 정리
    if request.project_id:
        deleted_staffing_res = await db.execute(
            select(Staffing).where(
                Staffing.project_id == request.project_id,
                Staffing.deleted_at.isnot(None),
            )
        )
    else:
        deleted_staffing_res = await db.execute(
            select(Staffing).where(Staffing.deleted_at.isnot(None))
        )
    deleted_staffings = deleted_staffing_res.scalars().all()
    deleted_ids = {s.id for s in deleted_staffings}

    if deleted_ids:
        soft_orphan_res = await db.execute(
            select(Calendar_entries).where(
                Calendar_entries.staffing_id.in_(deleted_ids)
            )
        )
        soft_orphans = soft_orphan_res.scalars().all()
        soft_count = len(soft_orphans)
        if soft_count > 0:
            detail.append(f"soft-deleted staffing의 entries: {soft_count}개 삭제")
            for so in soft_orphans:
                await db.delete(so)
                orphan_deleted += 1

    await db.commit()
    detail.append(f"총 {orphan_deleted}개 고아 entries 삭제 완료")

    return RepairScheduleResponse(
        checked_staffings=len(valid_ids),
        missing_mapped=0,
